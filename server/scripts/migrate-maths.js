// ============================================
// SCRIPT: MIGRAR PREGUNTAS DE MATEMÁTICAS (30 NIVELES) - VERSIÓN ESPAÑOL
// ============================================
require('dotenv').config();
const { pool, testConnection } = require('../config/database');

// Importar los niveles desde el archivo
const { levels } = require('../../materias/matematicas-data.js');

async function migrateMathsQuestions() {
    console.log('🚀 MIGRANDO PREGUNTAS DE MATEMÁTICAS (30 NIVELES) - ESPAÑOL');
    console.log('═══════════════════════════════════════════════\n');
    ...
    try {
        // 1. Obtener ID de la materia "Matemáticas"
        ...
        if (materiaRows.length === 0) {
            console.error('❌ Materia "Matemáticas" no encontrada');
            process.exit(1);
        }

        const materiaId = materiaRows[0].id;
        console.log(`📚 Materia encontrada: Matemáticas (ID: ${materiaId})`);

        // 2. Limpiar datos existentes
        console.log('\n🗑️ Eliminando datos existentes de Matemáticas...');
        ...
        for (const level of levels) {
            ...
            for (const q of level.questions) {
                // 🔥 SOLO ESPAÑOL: usar q.question.es
        ...
        console.log('\n🎉 ¡MIGRACIÓN DE MATEMÁTICAS COMPLETADA!');
